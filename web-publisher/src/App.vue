<template>
  <v-app>
    <v-app-bar dark extended flat fixed src="https://cdn.vuetifyjs.com/images/backgrounds/vbanner.jpg">
      ...
      <v-avatar id="menu-activator" color="white" size="40">
    ...
    <div class="system-alert">
      <v-alert transition="scroll-y-transition" :value="$store.state.alert.status" border="left" :type="$store.state.alert.type" :color="$store.state.alert.color" :icon="$store.state.alert.icon" dark>
        {{ $store.state.alert.msg }}
      </v-alert>
    </div>
    <v-menu left bottom activator="#menu-activator" transition="scroll-x-reverse-transition">
      <v-list>
        <v-list-item>
...
export default {
  ...
  created() {
    ...
    this.keycloak.loadUserProfile().success((profile) => {
      this.$store.commit("initUserProfile", profile);
    }).error(() => {
      this.$log.info("Fail to load user profile");
      this.$store.commit("cnvrtUserProfile", this.keycloak.tokenParsed);
    });
  },
  mounted() {
  ...
  methods: {
    onSelectedOption(opt) {
      switch(opt.keyIdx) {
        ...
        case 2: this.$api.get({nm: "sys-mng", ver: "v1"}); break;
